<template>
  <div class="calendario-container">
    ...
    <div class="header">
      ...
      <span>{{ meses[mesSeleccionado] }} {{ anioSeleccionado }}</span>
    ...
    <div v-for="habitacion in habitaciones" :key="habitacion.id" class="habitaciones">
      <div class="habitacionNombre">{{ habitacion.nombre }}</div>
      <div class="dias-grid">
        <div v-for="dia in diasMes" :key="dia" class="fechas"></div>

        <div v-for="reserva in reservasDeHabitacion(habitacion.id)" :key="reserva.id" class="reserva-bloque" :style="estiloReserva(reserva)">
          <!-- Aquí está la corrección: usar backticks para template literal -->
          <router-link :to="`/Reserva/${reserva.id}`" class="detalle-btn">
...
const mesSeleccionado = ref(dayjs().month());
...
const diasMes = computed(() => dayjs(`${anioSeleccionado.value}-${mesSeleccionado.value + 1}-01`).daysInMonth());
...
const estiloReserva = (reserva) => {
  ...
  const inicioMes = dayjs(`${anioSeleccionado.value}-${mesSeleccionado.value + 1}-01`);
  const finMes = inicioMes.endOf("month");

  if (finReserva.isBefore(inicioMes, "day") || inicioReserva.isAfter(finMes, "day")) {
    return { display: "none" };
  }

  const inicio = inicioReserva.isBefore(inicioMes, "day") ? 1 : inicioReserva.date();
  const fin = finReserva.isAfter(finMes, "day") ? finMes.date() : finReserva.date();
...
const mesAnterior = () => {
  const nuevaFecha = dayjs(`${anioSeleccionado.value}-${mesSeleccionado.value + 1}-01`).subtract(1, "month");
  mesSeleccionado.value = nuevaFecha.month();
  anioSeleccionado.value = nuevaFecha.year();
};

const mesSiguiente = () => {
  const nuevaFecha = dayjs(`${anioSeleccionado.value}-${mesSeleccionado.value + 1}-01`).add(1, "month");
  mesSeleccionado.value = nuevaFecha.month();
  anioSeleccionado.value = nuevaFecha.year();
};
</script>
